...
context: fork
allowed-tools: Bash, mcp__orca__get_asset_by_name, mcp__orca__discovery_search, mcp__orca__get_asset_alerts_count_grouped_by_risk_level, mcp__orca__get_asset_related_alerts_summary
---
...
**A)** `mcp__orca__discovery_search` with **exactly** this phrase:
```
...
**B)** `mcp__orca__get_asset_by_name` with:
...
1. `mcp__orca__get_asset_alerts_count_grouped_by_risk_level(asset_id=<id>)`
   → Returns counts per risk level (Critical / High / Medium / Low / Informational)

2. `mcp__orca__get_asset_related_alerts_summary(asset_id=<id>)`
